...
fn test_parse_api_response() {
    ...
    let vertices = &response.responses[0].text_annotations[0]
        .bounding_poly
        .as_ref()
        .unwrap()
        .vertices;
...
fn test_api_key_environment_variable() {
    ...
    let result =
        image_anonymizer::ocr::detection::detect_text_with_api(Path::new("nonexistent.jpg"));
    assert!(
        result.is_err(),
        "Function should error when GCP_API_KEY is not set"
    );

    // Check specific error message
    let err = result.unwrap_err().to_string();
    assert!(
        err.contains("GCP_API_KEY"),
